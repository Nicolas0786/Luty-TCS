...
const table = ({filterDados, dadosUsuario, head}) =>{
    ...
    return(
        <Table striped>
            ...
            <tbody>
                {filterDados == 0 ? dadosUsuario.map(record => 
                    <Row key={record.idUsuario} columns={columns} record={record}/>) : filterDados.map(record => 
                    <Row key={record.idUsuario} columns={columns} record={record}/>)}     
            </tbody>
        </Table>
    )
